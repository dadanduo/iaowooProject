...
public class ShopChooseGroupView extends LinearLayout {

    private Context context;

    public ShopChooseGroupView(Context context) {
        super(context);
        this.context = context;
        setOrientation(VERTICAL);//设置方向
    }

    public ShopChooseGroupView(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.context = context;
        setOrientation(VERTICAL);//设置方向
    }

    public ShopChooseGroupView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void initViews(String items[], final OnItemClick onItemClick) {
        ...
        for (int i = 0; i < size; i++) {//便利items
            ...
            final View view = LayoutInflater.from(context).inflate(R.layout.item_shop, null);
            final TextView itemView =  view.findViewById(R.id.text_shop);
            ...
            itemView.setOnClickListener(new OnClickListener() {//给每个item设置点击事件
                ...
                public void onClick(View v) {
                    if (null != onItemClick) {
                        ...
                        view.setBackgroundColor(R.drawable.item_selector_shop_select);
                    }
                }
            });
