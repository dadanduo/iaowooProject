 ...
 */
public class ClassInfo_TwoBarRecycleAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    //item类型
    public static final int ITEM_TYPE_HEADER = 0;
    public static final int ITEM_TYPE_CONTENT = 1;
    private EearthFragmentPresenter earthFragmentProsenter;
    private BigRecycleAdapter.OnRecyclerViewListener onRecyclerViewListener;
    ...
    private List<FenLei.BodyBean.ContentBean.ChildProductCategoryInfosBean> mData;
     ...
     */
    public void setOnRecyclerViewListener(BigRecycleAdapter.OnRecyclerViewListener onRecyclerViewListener) {
        this.onRecyclerViewListener = onRecyclerViewListener;
    }

    public ClassInfo_TwoBarRecycleAdapter(Context mContext, List<FenLei.BodyBean.ContentBean.ChildProductCategoryInfosBean> mData,ClassInfoPresenter classInfoProsenter,String categroy) {
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        if (holder instanceof HeaderViewHolder) {
            ...
            if (mData != null) {
                ((ContentViewHolder)holder).classinfo_two_Name.setText(mData.get(position-1).getName()+"");
                GlideUtils glideUtils=new GlideUtils();
                if(!TextUtils.isEmpty(mData.get(position-1).getImg())) {
                    glideUtils.glides(context, mData.get(position-1).getImg(), ((ContentViewHolder)holder).two_class_imge);
                }
                ((ContentViewHolder)holder).two_class_imge.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        ...
                        mintent.putExtra("title_name",mData.get(position-1).getName());
                        mintent.putExtra("categroy",mData.get(position-1).getParentCode());
