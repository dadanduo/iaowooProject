        ...
        XutilsHttp.getInstance().postEncodedGetLoginBysms(new OkhttpCallBack() {
            ...
            public void OnSuccess(String json) {
                ...
                if (response.getCode().equals(OkhttpManager.SUCESS)) {
                    ...
                    String loginToken = response.getBody().getContent().getLoginToken();
                    String inviteCode = response.getBody().getContent().getInviteCode();

                    if (!TextUtils.isEmpty(loginToken)) {
                        PrefManager.getInstance().setPassWordLogin(false);
                        //把token存起来
                        PrefManager.getInstance().setToken(loginToken);
                        if (!TextUtils.isEmpty(inviteCode)) {
                            //把邀请码存起来
                            PrefManager.getInstance().setInvite(inviteCode);
                        }

                        // 通过loginToken连接RongIM
//                        loginCallBack.connetRongIM(loginToken);
